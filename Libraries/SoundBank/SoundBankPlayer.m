...
- (void)tearDownAudio
{
	if (_initialized)
	{
        ...
        // When reusing the soundbankplayer, cannot kill the context and device without ruining
        // it for every other player
...
- (void)loadSoundBank:(NSString *)filename
{
    ...
    NSDictionary *instrument = [NSDictionary dictionaryWithContentsOfFile:path];
	NSArray *array = [instrument valueForKey:@"data"];
	if (array == nil)
	{
...
- (void)dropQueuedNotes
{
    for (int t = 0; t < NUM_SOURCES; ++t)
    {
        if (_sources[t].queued) _sources[t].queued = NO;
    }
}
